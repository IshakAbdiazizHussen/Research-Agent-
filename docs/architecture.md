...
| Web search tool | **OpenAI `web_search` (Responses API)** — `tools/web_search_openai.py`, the sole/permanent implementation. | Tavily's marketing/dashboard site (tavily.com, api.tavily.com root) was unreliably reachable from this network during Feature 2 setup, blocking sign-up; OpenAI's built-in web_search tool works through the `OPENAI_API_KEY` already funded for the LLM calls, so no second provider/key was needed to unblock the feature. Real measured cost: **≈$0.0855 for 8 calls** in a live test run (2 eval queries + 1 manual retry-loop check); ongoing rate is **$10.00/1,000 calls + search-content tokens billed at standard model rates** (gpt-4o-mini: $0.15/1M input, $0.60/1M output) — notably pricier per call than Tavily's ~$0.008/basic-search credit (or free under 1,000 credits/month). Response shape also differs materially from a typical raw-result search API: OpenAI returns a synthesized answer with citation annotations, not independent raw search results — see `tools/web_search_openai.py`'s module docstring for how that's adapted to the `{title, url, content}` shape the Grader node expects. **Final note — accepted baseline, not reopened, not pending:** measured via Feature 7's eval harness (`tests/eval/run_eval.py`) against the real 20-question set: **25% groundedness, 20% judged-correct, 80% retry-cap-exhaustion**. This falls short of `project_definition.md`'s ≥80% groundedness/correctness success criteria. That gap is accepted as a known limitation of this search tool choice, not a defect elsewhere in the system — root cause isolated and confirmed: OpenAI's `web_search` skips citing sources whenever the model is confident in its own knowledge, independent of query type (not limited to "broad conceptual" questions, as an earlier, too-narrow ~5-example characterization had claimed). Both alternatives were seriously evaluated before landing here, not dismissed reflexively: **Brave** — signup path scoped, cost analyzed — declined not on cost but because adding a second recurring API-key dependency wasn't worth it at this project stage, given one key (OpenAI's) is already funded and in use everywhere else. **Tavily** — ruled out entirely on reachability, not merit: a persistent, unresolved CloudFront block across the whole `app.tavily.com` subdomain, confirmed via 5 separate attempts through different paths, all failing identically. | A second search-tool budget/dependency becomes justified by project scale or requirements, **or** Tavily becomes reachable, **or** OpenAI changes this tool's behavior. Until one of those happens, the 25%/20%/80% baseline above is the accepted, documented state of this feature — not a bug to chase. |
